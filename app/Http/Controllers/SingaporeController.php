...
class SingaporeController extends Controller
{
    public function download($id)
    {
        ...
        // Periksa apakah 'code_sgp' ada di model Marketing
        if (empty($marketing->code_sgp)) {
            return response()->json(['error' => 'code_sgp pada Marketing tidak valid atau kosong.'], 400);
        }

        // Render tampilan ke HTML
        $htmlContent = View::make('singapore', [
            'marketing' => $marketing,
        ...
        $filename = $marketing->code_sgp . '_' . $appName . '_SGP_' . $timestamp . '.pdf';
