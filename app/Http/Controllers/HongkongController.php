...
class HongkongController extends Controller
{
    public function download($id)
    {
        ...
        // Periksa apakah 'code_hk' ada di model Marketing
        if (empty($marketing->code_hk)) {
            return response()->json(['error' => 'code_hk pada Marketing tidak valid atau kosong.'], 400);
        }

        // Render tampilan ke HTML
        $htmlContent = View::make('hongkong', [
            'marketing' => $marketing,
        ...
        $filename = $marketing->code_hk . '_' . $appName . '_HK_' . $timestamp . '.pdf';
